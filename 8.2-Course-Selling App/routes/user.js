...
userRouter.post('/signup', async(req,res)=>{
    ...
    const {email,password,firstName,lastName} = req.body;  //// Destructure req.body 
     ...
     const hashedPassword = await bcrypt.hash(password, 10);
      ...
      try {
        ...
        return res.status(400).json({
            ...
            // // As the database schema has already been defined as unique email, so it will throw an error and will be caught by catch block 
        });
    }
...
userRouter.post("/signin", async function (req, res) {
    ...
    if (passwordMatch) {
        ...
        res.status(403).json({
            message: "incorrect Credentials!", // Error message for failed password comparison
        });
    }
});
